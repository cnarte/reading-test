...
def predict_vid(vid):
    ...
    if ((averageFixationLengthMS > 200) and (averageFixationLengthMS < 330)):
        
        return {"result": 'PROBABLY NOT DYSLEXIC',"averageFixationLengthMS":averageFixationLengthFRAMES}
        
    if ((averageFixationLengthMS > 330) and (averageFixationLengthMS < 350)):
        
        return {"result": 'PROBABLY DYSLEXIC',"averageFixationLengthMS":averageFixationLengthFRAMES}

import time

start = time.time()
predict_vid("https://fileuploadapp.blob.core.windows.net/tutorial-container/aman.mp4")
end = time.time()
